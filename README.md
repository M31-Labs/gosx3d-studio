...
`go.mod` pins GoSX `v0.57.0` and Arbiter `v1.9.0`, and `go.sum` checksums the
...
sockets remain Standard PBR. GoSX v0.57.0 keeps selected PBR surfaces solid
...
```bash
go run m31labs.dev/gosx/cmd/gosx@v0.57.0 check app/page.gsx
