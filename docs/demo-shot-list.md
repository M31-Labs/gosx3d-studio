...
| --- | --- | --- |
| 0:00-0:22 | Establish the clean scene prepared before capture without selecting Board | Baseline `[R]`, **Agent tools ready**, **4 tools**, agent stages / human applies boundary |
| 0:22-0:58 | Copy and send the exact in-product prompt | Inspect/Find/Focus/Stage completion, persistent typed-call results, agent-found stable ID `board`, visible focus |
| 0:58-1:28 | Inspect the staged review card | Human-readable rename and material changes, `agent://webmcp`, Arbiter Allow 2/2, fingerprint, canonical `[R]` unchanged |
| 1:28-1:42 | Reload the same browser tab | Typed-call trace, proposal, and Apply/Discard return; canonical revision still `[R]` |
| 1:42-2:08 | Human reviews, then clicks **Apply staged changes** | `Launch Board`, `Cobalt Pieces`, revision `[R+1]` |
| 2:08-2:16 | Show Agent Activity and finish on Studio | Agent proposal and human approval attribution together |
...
- **Layout:** Put the Studio and browser agent in one frame. Use a tested zoom
  ...
  persistent typed-call trace, proposal card, and Agent Activity readable
  without live resizing.
...
4. Reset the shared scene before recording, write down baseline `[R]`, confirm
   no proposal is staged, and leave Board unselected.
...
- Keep the final timeline between 2:10 and 2:20. Watch the exported MP4 from
